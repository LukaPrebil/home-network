...
- Standalone matterjs-server has no host-clock guard (the add-on's `auto` mode is add-on-only); it pushes the host clock unconditionally. Managed NTP on rpi4 is therefore a hard prerequisite, delivered as an Ansible-managed timesyncd config driven by a central `ntp_servers` variable - which is also the seam for a future local time server (planned rpi2 + GPS): repoint the variable, converge. Amended 2026-09-24: the managed daemon is now chrony on every host with its own clock (inventory group chrony_hosts), still driven by ntp_servers, because timesyncd follows a single server and tracked a pool server that was 5.7 ms off.
